...
LUA_SHA1 = get_hash(os.path.join(CWD, 'radiofreeasia.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            item['raw_items'].append(item_name)
            item_type, item_value = item_name.split(':', 1)
            if item_type == 'article':
                site, path = item_value.split(':', 1)
                full_name = site
                if full_name == 'rfa':
                    full_name = 'radiofreeasia'
                site = 'www.{}.{}'.format(site, 'me' if site == 'wainao' else 'org')
                wget_args.extend(['--warc-header', '{}-path: {}'.format(full_name, path)])
                wget_args.append('https://{}/{}'.format(site, path))
            elif item_type == 'video':
                wget_args.extend(['--warc-header', 'video-id: '+item_value])
                wget_args.append('https://radiofreeasia-prod-cdn.video-api.arcpublishing.com/api/v1/ansvideos/findByUuid?uuid={}&cb=powaCallback{}'.format(item_value, item_value.replace('-', '')))
            elif item_type == 'asset':
                url = 'https://' + item_value
                wget_args.extend(['--warc-header', 'asset: '+url])
                wget_args.append(url)
            else:
                raise Exception('Unknown item')

        item['item_name_newline'] = item['item_name'].replace('\0', '\n')
        item['raw_items'] = json.dumps(item['raw_items'])
